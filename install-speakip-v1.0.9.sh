...
#   - Uses asl-tts -f to create a temporary .ul file, then plays it with rpt playback.
#
...
sub speak_text {
    ...
    # Clean up old Speak IP generated TTS files.
    system('/usr/bin/find', '/tmp', '-maxdepth', '1', '-name', 'speakip-*.ul', '-mmin', '+60', '-delete');

    my $base = '/tmp/speakip-' . time() . '-' . $$;
    logger("Creating TTS file for node $node at $base.ul");

    # Per ASL3 docs, -f should be given without an extension.
    # asl-tts appends .ul automatically.
    my $result = system($tts, '-n', $node, '-t', $text, '-f', $base);
    if ($result != 0) {
        logger("asl-tts file creation failed for node $node with exit status $result.");
        return 0;
    }

    if (!-r "$base.ul") {
        logger("asl-tts reported success but $base.ul was not found.");
        return 0;
    }

    logger("Playing generated TTS file for node $node: $base");
    my $play_ok = play_prompt($node, $base);
    if (!$play_ok) {
        logger("rpt playback failed for generated TTS file: $base");
        return 0;
    }
