...
use SpeakIP qw(load_config play_prompt speak_text spoken_ipv4 valid_ipv4 logger);
...
ExecStart=/etc/asterisk/local/sayip.pl
TimeoutStartSec=60
...
run_test_prompt() {
  ...
  if [[ -t 0 ]]; then
    ...
    answer="${answer:-Y}"
    if [[ "$answer" =~ ^[Yy]$ ]]; then
      cd / && sudo -u asterisk "${LOCAL_DIR}/sayip.pl" || warn "The test announcement returned an error. Check: journalctl -t speakip"
    fi
  fi
}
